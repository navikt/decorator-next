...
((self) => {
                    ...
                    (_c =
                        null ===
                            (_b =
                                null == obj
                                    ? void 0
                                    : obj.__zone_symbol__originalInstance) ||
                        void 0 === _b
                            ? void 0
        ...
        getterDimensionPropNames.map((propName) =>
            definePrototypeProperty(Cstr, propName, {
                get() {
                    ...
                    if (
                        groupedDimensions &&
                        "object" == typeof groupedDimensions
                    ) {
    ...
    const deserializeFromMain = (
        ...
        if (serializedValueTransfer) {
            ...
            if (7 === serializedType) {
                return new NodeList(
                    serializedValue.map(getOrCreateSerializedInstance),
                );
            }
    ...
    const callMethod = (
        ...
        callType =
            callType || (nonBlockingMethods.includes(methodName) ? 2 : 1);
        if (
    ...
    const constructGlobal = (instance, cstrName, args) => {
        ...
        queue(
            instance,
            [1, cstrName, serializeInstanceForMain(instance, args)],
            1,
        );
    };
                    ...
                    return new Proxy(this, {
                        get(target, propName) {
                            ...
                            target[propName] ||
                                ...
                                (target[InstanceDataKey][propName] = getter(
                                    target,
                                    [propName],
                                ));
                        ...
                        ((winId, iframe) => {
                            ...
                            let callback = () => {
                                if (
                                    ...
                                    type = getInstanceStateValue(iframe, 1)
                                        ? "error"
                                        : "load";
        ...
        const DocumentDescriptorMap = {
            ...
            createElement: {
                value(tagName) {
                    ...
                    const elm = getOrCreateNodeInstance(
                        winId,
                        instanceId,
                        tagName,
                    );
                ...
                set(src) {
                    if (src) {
                        if (src.startsWith("javascript:")) {
                            ...
                            env.$location$.href = src = resolveUrl(
                                env,
                                src,
                                "iframe",
                            );
    ...
    const createWindow = (
        ...
        const WorkerBase = class {
            constructor(winId, instanceId, applyPath, instanceData, namespace) {
                ...
                this[InstanceIdKey] =
                    instanceId || cstrInstanceId || randomId();
        ...
        const WorkerWindow = defineConstructorName(
            class extends WorkerBase {
                constructor() {
                            ...
                            (() => {
                                if (!webWorkerCtx.$initWindowMedia$) {
                                    ...
                                    webWorkerCtx.$importScripts$(
                                        partytownLibUrl(
                                            "partytown-media.js?v=0.10.2",
                                        ),
                                    );
                                    webWorkerCtx.$initWindowMedia$ =
                    ...
                    ((win) => {
                        win.NodeList = defineConstructorName(
                            NodeList,
                            "NodeList",
                        );
                    })(win);
                    createNamedNodeMapCstr(win, WorkerBase);
                    createCSSStyleDeclarationCstr(
                        win,
                        WorkerBase,
                        "CSSStyleDeclaration",
                    );
                    ...
                    if (isIframeWindow) {
                        ...
                        const originalPushState = win.history.pushState.bind(
                            win.history,
                        );
                ...
                get navigator() {
                    return ((env) => {
                        const nav = {
                            sendBeacon: (url, body) => {
                                if (
                                    webWorkerCtx.$config$.logSendBeaconRequests
                                ) {
                                    try {
                                        logWorker(
        ...
        eventTargetMethods.map(
            (methodName) =>
                (WorkerEventTargetProxy.prototype[methodName] = function (
                    ...args
                ) {
    ...
    const receiveMessageFromSandboxToWorker = (ev) => {
        ...
        if (webWorkerCtx.$isInitialized$) {
            if (7 === msgType) {
                (async (initScript) => {
                    ...
                    if (scriptSrc) {
                        try {
                            scriptSrc =
                                resolveToUrl(env, scriptSrc, "script") + "";
                            setInstanceStateValue(instance, 4, scriptSrc);
                            webWorkerCtx.$config$.logScriptExecution &&
                                logWorker(
                                    `Execute script src: ${scriptOrgSrc}`,
                                    winId,
                                );
                            ...
                            if (rsp.ok) {
                                ...
                                let shouldExecute = javascriptContentTypes.some(
                                    (ct) => {
                                        var _a, _b, _c;
                                        return null ===
                                            (_c =
                ...
                if (1 !== environments[msgValue].$isInitialized$) {
                    ...
                    const winType =
                        env.$winId$ === env.$parentWinId$ ? "top" : "iframe";
                    logWorker(
            ...
            ((initWebWorkerData) => {
                ...
                webWorkerCtx.$sharedDataBuffer$ =
                    initWebWorkerData.$sharedDataBuffer$;
