...
((window) => {
                  ...
                  : (added = added || new Set()) && Array.isArray(value)
                    ? added.has(value)
                        ...
                        : added.add(value) && [
                              1,
                              value.map((v) =>
                                  serializeForWorker($winId$, v, added),
                              ),
                          ]
                    : "object" === type
        ...
        if (!added.has(obj)) {
            ...
            for (propName in obj) {
                if (isValidMemberName(propName)) {
                    propValue =
                        "path" === propName &&
                        getConstructorName(obj).endsWith("Event")
                            ? obj.composedPath()
                            : obj[propName];
    ...
    const serializeCssRuleForWorker = (cssRule) => {
        ...
        let key;
        for (key in cssRule) {
            validCssRuleProps.includes(key) &&
        ...
        if (serializedTransfer) {
            ...
            return 0 === serializedType
                ...
                : 4 === serializedType
                  ...
                  : 1 === serializedType
                    ? serializedValue.map((v) =>
                          deserializeFromWorker(worker, v),
                      )
                    : 3 === serializedType
                      ? getInstance(serializedValue[0], serializedValue[1])
                      : 5 === serializedType
                        ? constructEvent(
                              deserializeObjectFromWorker(
                                  worker,
                                  serializedValue,
                              ),
                          )
                        : 2 === serializedType
                          ...
                          : 8 === serializedType
                            ? serializedValue
                            : 9 === serializedType
                              ? new window[serializedTransfer[2]](
                                    serializedValue,
                                )
                              : void 0;
        }
            ...
            try {
                if (!Array.isArray(next)) {
                    if (
                        ...
                        if (0 === next) {
                            instance[previous] = deserializeFromWorker(
                                worker,
                                current,
                            );
                            return;
    ...
    const readNextScript = (worker, winCtx) => {
        ...
        let blockingScriptSelector =
            scriptSelector + ":not([async]):not([defer])";
        let scriptElm;
        let $instanceId$;
        let scriptData;
        if (doc && doc.body) {
            ...
            if (scriptElm) {
                ...
                if (scriptElm.src) {
                    ...
                    scriptData.$orgUrl$ =
                        scriptElm.dataset.ptsrc || scriptElm.src;
                } else {
    ...
    const registerWindow = (worker, $winId$, $window$) => {
        if (!windowIds.has($window$)) {
            ...
            winCtxs[$winId$].$startTime$ = performance.now();
            {
                ...
                logMain(
                    `Registered ${winType} window ${normalizedWinId($winId$)}`,
                );
            }
    ...
    const readOwnImplementation = (
        ...
        if (!cstrs.has(cstrName)) {
            ...
            const propDescriptors =
                Object.getOwnPropertyDescriptors(CstrPrototype);
            readOwnImplementation(
                cstrs,
                interfaces,
                superCstrName,
